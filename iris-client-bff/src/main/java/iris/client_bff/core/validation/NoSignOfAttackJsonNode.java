...
@Retention(RUNTIME)
@Constraint(validatedBy = NoSignOfAttackJsonNode.NoSignOfAttackJsonNodeValidator.class)
@Documented
public @interface NoSignOfAttackJsonNode {
	...
	static class NoSignOfAttackJsonNodeValidator implements ConstraintValidator<NoSignOfAttackJsonNode, JsonNode> {
		...
		@Override
		public void initialize(NoSignOfAttackJsonNode constraintAnnotation) {
		...
		@Override
		public boolean isValid(final JsonNode node, final ConstraintValidatorContext context) {
			...
			try {
				...
				if (context instanceof ConstraintValidatorContextImpl impl) {
					path = impl.getConstraintViolationCreationContexts()
							.get(0)
							.getPath()
							.asString();
				}
